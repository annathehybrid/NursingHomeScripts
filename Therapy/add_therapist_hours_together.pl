...
#	Disc (shows whether the therapist is a physical, speech, or occupational therapist)
...
# 	Therapist name
#	Minutes worked
# with columns of insurance

# open the raw csv file from therapist
my $filename_raw = 'anna2.csv';
...
# go through the table from the headers to the last line
# although the last line will contain some empty values
...
#split each line by comma
	...
	if (length ($data[$i]) > 25) {
		...
		# when we split this one, the therapist name (Last name, first name) will be split again
		# so we have to join them
		my $therapist_name = join (',', $data_cells[$number_therapist_name], $data_cells[$number_therapist_name + 1]);
		
		# the split of the therapist name field will also push the minutes worked one place
